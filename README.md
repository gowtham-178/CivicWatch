...
- 📈 **Dynamic Analytics Dashboard**:
  - **Live Charts**: Pie charts displaying percentage breakdown on hover with bottom legend support to prevent text clipping, area trend charts, and vertical workload distribution bar charts.
  - **Dynamic Average Response Time**: Intelligently scales and displays in **hours** for quick turnarounds or **days** if resolution takes more than 24 hours.
  - **Automated Performance Insights**: Identifies underperforming categories (resolution rate < 70%) and flags critical backlogs.
- 🔍 **Advanced Filtering**: Categorize, filter, and search all community issues instantly.
- 👥 **User Management & Role Assignment**: Modify user roles (User/Admin) in bulk.
- 🏢 **Department Workloads**: Assign and track issues routed to departments (e.g. sanitation, public works).
